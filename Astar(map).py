def Astar(start, end):
    ...
    visited = set()       
    g_n = {node: float('inf') for node in graph}                                          # Distance from start node to each node (g(n))
    g_n[start] = 0
    ...
    while queue:
        queue.sort(key=lambda x: x[0])                                      # Sort the queue based on f(n) = g(n) + h(n) and get the node with the lowest f(n)
        ...
        for adjacent in graph[node]['Adjacents']:
            if adjacent not in visited:
                ...
                if temp_n < g_n[adjacent]:              #jis trha dijsktra me hua tha
                    g_n[adjacent] = temp_n
